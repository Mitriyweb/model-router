...
beforeEach(() => {
  ...
  originalApiKey = config.groq.apiKey;
...
describe("Groq Adapter", () => {
  it("should handle basic request", async () => {
    globalThis.fetch = mock(async () =>
      makeOkResponse("Hello from Groq!"),
    ) as unknown as typeof fetch;

    const response = await groqAdapter.send(baseReq);
    expect(response).toBeDefined();
    expect(response.role).toBe("assistant");
    expect(response.content[0].type).toBe("text");
    expect((response.content[0] as any).text).toBe("Hello from Groq!");
  });
  ...
  it("should handle streaming requests", async () => {
    ...
    const stream = await groqAdapter.sendStream!(baseReq);
